...
proc init_job {job} {
	...
	#set stats_file [format "stats_%s" [$job job_id]]
	set stats_file "/dev/null"
	$job set stats [open $stats_file w]
}
...
foreach job $jobs_list {
	if {[$job set args] == "{}" } {
		submit_job 0 [$job set type]
	} else {
		submit_job 0 [$job set type] [$job set args]
	}
}
